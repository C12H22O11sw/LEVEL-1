...
public class TurtlePond implements KeyEventDispatcher {
	...
	private void checkForFood() throws Exception {
		...
		if(Math.sqrt(Math.pow(tortoiseLocationX - cookieX ,2)+Math.pow(tortoiseLocationY - cookieY,2))< 100){
			setBackgroundColor(Color.YELLOW);
		}
		// 8. If the Tortoise is within 50 pixels of the food, set the background color to orange
		if(Math.sqrt(Math.pow(tortoiseLocationX - cookieX ,2)+Math.pow(tortoiseLocationY - cookieY,2))< 50){
			setBackgroundColor(Color.ORANGE);
		}
		// 9. If the Tortoise is within 20 pixels of the food, set the background color to red
		if(Math.sqrt(Math.pow(tortoiseLocationX - cookieX ,2)+Math.pow(tortoiseLocationY - cookieY,2))< 20){
			setBackgroundColor(Color.RED);
		}
		// 10. If the Tortoise is within 5 pixels of the cookie, make a pop-up to tell them they found it
		if(Math.sqrt(Math.pow(tortoiseLocationX - cookieX ,2)+Math.pow(tortoiseLocationY - cookieY,2))< 5){
